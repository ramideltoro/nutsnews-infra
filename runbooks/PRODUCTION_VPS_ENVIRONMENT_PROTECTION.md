...
2. Disable **Allow administrators to bypass configured protection rules**.
   Administrator bypass weakens the same approval boundary and is a rollout
   blocker even when reviewers and self-review prevention are configured.
...
5. Do not copy reviewer identities or IDs into workflow output, artifacts, or
   repository files.
...
3. Require the audit to report that exact-main policy, a required reviewer,
   self-review prevention, and disabled administrator bypass are present before
...
makes reviewer configuration unavailable for the repository plan, the audit fails closed.
