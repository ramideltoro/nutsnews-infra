...
The role can also install Grafana Alloy and a read-only textfile metrics timer when `enable_grafana_alloy` is explicitly set in the protected workflow. Grafana Cloud telemetry endpoints, usernames, and Access Policy token values come only from the protected `production-vps` Environment. Docker log collection is enabled for NutsNews Compose projects through the Docker API socket, while Docker/cAdvisor container metrics stay disabled by default until that separate privilege boundary is reviewed and documented.
