...
require("NUTSNEWS_VERCEL_API_TOKEN: ${{ secrets.NUTSNEWS_VERCEL_API_TOKEN }}" in TEXT, "Workflow must use the protected Vercel token only through env.")
require("NUTSNEWS_VERCEL_USAGE_API_URL: ${{ secrets.NUTSNEWS_VERCEL_USAGE_API_URL }}" in TEXT, "Workflow must use the protected Vercel URL only through env.")
require("matched_aggregates" in TEXT, "Workflow must print sanitized Vercel aggregate diagnostics.")
require("tag_keys" in TEXT, "Workflow must print only Vercel tag keys, not tag values.")
require("provider_keys" in TEXT, "Workflow must print sanitized provider keys when Vercel is missing.")
require("free_tier_errors" in TEXT, "Workflow must print sanitized collector errors when Vercel is missing.")
require("exit_code=3 if missing else (2 if" in TEXT, "Workflow must distinguish missing Vercel from cached Vercel usage.")
require("source_status" in TEXT and "cached" in TEXT, "Workflow must fail explicitly on cached Vercel usage.")
...
require("print(token" not in TEXT, "Workflow must not print provider API tokens.")
require("print(url" not in TEXT, "Workflow must not print provider API URLs.")
require("ProjectName" not in TEXT, "Workflow must not print Vercel project names.")
require("ProjectId" not in TEXT, "Workflow must not print Vercel project IDs.")
