...
def main() -> None:
    ...
    require("ops.nutsnews.com {" in caddyfile, "Caddy must expose the public Ops Portal hostname.")
    public_portal_block = caddyfile.split("ops.nutsnews.com {", 1)[1].split("\n}", 1)[0]
    require("reverse_proxy ops-auth:8090" in caddyfile, "Caddy must proxy portal traffic through auth gateway.")
    require("reverse_proxy ops-auth:8090" in public_portal_block, "Public portal host must proxy through auth gateway.")
    require("import /etc/nutsnews/caddy/app.routes" not in public_portal_block, "Public portal host must not import unauthenticated app routes.")
    ...
    require("vps_service_foundation_cloudflare_ddns_record_names:" in defaults, "Cloudflare DDNS record list is missing.")
    require("- ops.nutsnews.com" in defaults, "Cloudflare DDNS must manage the public Ops Portal hostname.")
    ...
    require("caddy-oauth-routing-reload-marker" in tasks, "Caddy OAuth routing rollout must have a one-time reload marker.")
