#!/usr/bin/env python3
"""Validate automatic Grafana Cloud apply workflow guardrails."""
...
require(
    ...
    "Grafana Cloud apply must run automatically on push.",
)
require(
    re.search(r"(?ms)^  push:\s*$.*?^    branches:\s*$.*?^      - main\s*$", TEXT)
    is not None,
    "Grafana Cloud apply push trigger must target main.",
)
for required_path in (
    '".github/workflows/grafana-cloud-apply.yml"',
    '"terraform/grafana-cloud/**"',
):
    require(required_path in TEXT, f"Grafana Cloud apply push paths are missing {required_path}.")
require(
    re.search(r"(?m)^  workflow_dispatch:\s*$", TEXT) is not None,
    "Grafana Cloud apply must retain an exact-main manual retry.",
)
...
require(
    "environment: production-vps" in TEXT,
    "Grafana Cloud apply must use the production-vps environment for secrets.",
)
...
require(
    "github.repository == 'ramideltoro/nutsnews-infra'" in workflow_job(TEXT, "apply"),
    "Apply must be restricted to the canonical repository.",
)
require(
    "github.ref == 'refs/heads/main'" in workflow_job(TEXT, "apply"),
    "Apply must be restricted to exact main.",
)
require("confirm_apply" not in TEXT, "Automatic apply must not require a manual confirmation input.")
require(
