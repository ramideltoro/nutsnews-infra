...
require("skip_install_trust" in CADDY, "Caddy internal TLS must not try to install its local CA root in the hardened container.")
require("nutsnews.com, www.nutsnews.com" in CADDY, "Caddy must define the production primary hostnames.")
require("tls internal" in CADDY, "Primary hostname origin TLS must be available before DNS cutover.")
require("@apex host nutsnews.com" in CADDY, "Caddy must keep apex canonical redirect handling.")
require("redir https://www.nutsnews.com{uri} 308" in CADDY, "Caddy must preserve the apex-to-www redirect.")
...
require('"0.0.0.0:80:80/tcp"' in COMPOSE, "Compose must publish public HTTP on IPv4 for ACME and redirects.")
require('"0.0.0.0:443:443/tcp"' in COMPOSE, "Compose must publish public HTTPS on IPv4 for Better Stack.")
