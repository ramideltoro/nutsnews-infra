...
Use the protected `Cloudflare Cache Rules Apply` workflow. Run `cache_policy_mode=baseline` first to import and verify the active two-rule ruleset without intended drift. After that plan is clean, use `cache_policy_mode=coordinated` to review or apply the long-lived policy; apply is gated by an exact confirmation. Cache keys retain only functional parameters, Next.js router variants bypass edge storage, optimized-image format negotiation normalizes `Accept` to the supported AVIF and WebP media types, and protected/mutating routes remain uncacheable.
