...
The 24 vendor alerts are not exempt from the universal NutsNews label and annotation contract. `grafana_rule_group.linux_integration_alert_replacements` provisions exact source-reviewed equivalents with normalized `severity`, `owner`, `route`, `service`, `deployment_environment`, dashboard, and runbook context. After a protected Grafana apply creates them, the protected `Grafana Linux Integration Alert Migration` workflow compares every live vendor definition with its committed replacement, verifies all 24 replacements and all 16 recording rules, and then uses the integration's supported `configurable_alerts` control to disable only the vendor alert bundle. It leaves logs and recording rules enabled and verifies the final state. Any UID, identity, provenance, source-severity, query, context, or integration-version drift fails before mutation. Separately, the catalog starts with `baselineStatus=pending_authenticated_rollout`: authenticated verification exports deterministic definition hashes and fails closed until an operator reviews and commits baselines for all 11 long-term retained recording rules.
