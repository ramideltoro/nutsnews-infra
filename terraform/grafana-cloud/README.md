...
latency, stage success/latency, active-owner ingestion freshness, newest
published-content age, retry/DLQ budget, final publication success, alert
state, canary fixtures, and recovery proof age.
...
The check map must contain exactly `canonical_homepage`, `canonical_readiness`, `canonical_articles_api`, `vps_readiness`, and `vercel_secondary_readiness` and use exactly two unique public probes. Source-controlled policy runs the three canonical checks every five minutes and the direct-VPS and Vercel-secondary readiness checks every ten minutes. The three canonical checks share one host; direct VPS and Vercel-secondary readiness use two other distinct hosts. Targets are credential-free, query-free HTTPS URLs on port 443 with exact approved paths. Every check must assert response content or headers in addition to status. Refresh, controller, ingestion, trigger, and publication routes are rejected. Checks stay in Grafana's default Synthetic Monitoring folder; do not assign the general NutsNews dashboard folder because Synthetic Monitoring only supports its own folder tree.
...
Keep real targets in protected variables or untracked local tfvars. The protected frequency field remains at its legacy five-minute value so existing secret material does not need to be rewritten, while the source-controlled map is authoritative for live cadence. The mixed production policy projects to 69,120 executions in a 30-day month, or 69.12% of the 100,000-execution allowance. The module blocks at or above the lower of 90% of the configured free API allowance and the absolute 90,000-execution ceiling. The protected validator emits only counts, effective interval bounds, the projected execution total, thresholds, and the deprecated acknowledgment state; it never emits check names, targets, probe IDs, or credentials.
...
`grafana_contact_point.operations_email` reuses `NUTSNEWS_EMAIL_TO`, sends firing and resolved notifications, and is protected from destroy. The global managed policy keeps initial delivery prompt while limiting reminders: `critical` groups after 30 seconds and repeats every four hours, `major` groups after two minutes and repeats every 12 hours, and `warning|minor|low` groups after five minutes and repeats every 24 hours. Unmatched alerts use the same 24-hour repeat. Every Terraform-managed rule carries bounded owner, route, service, severity, environment, dashboard, and runbook context. Post-apply verification uses the six-hour Loki window for active services and a bounded 24-hour window for quiet PostgreSQL, VPS Caddy, and web process sources; PostgreSQL evidence accepts its normalized journal or managed log-file source instead of manufacturing heartbeat log lines.

`grafana_slo.nutsnews` creates public availability (99.5%), API latency (95% of successful checks within 750 ms), active-owner ingestion freshness (99% of valid observations with a successful scheduled cycle within 15 minutes), and worker terminal success (99%) objectives over 30 days. Public and API SLOs use Grafana's documented gauge execution ratios over `$__interval`; ingestion freshness routes between the legacy scheduled-success age and the worker-uplift scheduler-loop signal according to the protected production owner. Quiet upstream publishers therefore do not consume the ingestion error budget. The separate newest-published-content panel and three-hour critical alert retain end-user content-age visibility. The API latency denominator contains only successful article-API checks; failed article-API probes trigger the all-check synthetic operational alert and do not dilute the latency objective. Fast/slow burn alerts are generated for the first three and authenticated verification checks their canonical critical (5m/30m/1h/6h) and warning (2h/6h/1d/3d) query families. Worker terminal burn alerts stay disabled while the split worker path is shadow-only.

The `integration---linux-node` inventory distinguishes 24 vendor alert rules from 16 recording rules. Eleven node-exporter recording rules are long-term retained and integration-owned. Five legacy `asserts-node.rules` recording rules remain temporarily because the authenticated live integration API reports Linux Server 1.6.2 installed, 1.6.2 available, and `has_update=false`; the expected 1.6.3 upgrade is not offered to this stack. After alert migration, the verifier accepts the exact 16-rule alerts-disabled shape or the exact 11-rule supported post-upgrade shape and rejects the initial 40-rule shape as incomplete migration. Never delete recording rules by UID. Re-review the catalog and use Grafana's supported integration upgrade once the live API offers it.

The 24 vendor alerts are not exempt from the universal NutsNews label and annotation contract. `grafana_rule_group.linux_integration_alert_replacements` provisions exact source-reviewed equivalents with normalized `severity`, `owner`, `route`, `service`, `deployment_environment`, dashboard, and runbook context. After a protected Grafana apply creates them, the protected `Grafana Linux Integration Alert Migration` workflow compares every live vendor definition and the integration's Prometheus rule bundle with the committed replacements, verifies all 24 replacements and all 16 recording rules, sets `configurable_alerts.alerts_disabled=true`, and reconciles Grafana's converted namespace `Integration - Linux Node` to the two recording-rule groups only. The conversion request carries the fixed `grafanacloud-prom` datasource identity. Any post-delete failure automatically restores the reviewed full 40-rule snapshot and the prior integration configuration, then verifies the rollback before failing. Logs remain enabled, recording UIDs and definitions are verified after the mutation, and repeat applies are no-ops. Any UID, identity, provenance, source-severity, query, context, namespace, datasource, or integration-version drift fails before mutation. The 11 long-term retained recording rules now have an approved authenticated definition-fingerprint baseline from protected apply run `30744049717`; future verification fails closed on identity, query, health, or definition drift.

Loki post-apply verification uses `/loki/api/v1/series` as the authoritative indexed-label inventory. It requires the six normalized Alloy boundary labels plus Grafana Cloud's `service_name` alias (which must equal `service`); request, correlation, trace, article, feed, message, and idempotency identifiers remain structured metadata and are not mistaken for indexed labels when query responses merge metadata into streams.

The backend snapshot-based Supabase sync relay is deliberately suspended by the protected backend baseline until reviewed incremental replication replaces it. Its textfile telemetry exports `expected_active=0`, a one-hot `not_configured` status, and bounded sentinel values so dashboards display “Disabled by configuration.” Post-apply verification requires that exact pairing and suppresses configured-only lag, freshness, and table-health checks; if a future reviewed deployment exports `expected_active=1`, those checks immediately become mandatory and fail closed.

All dashboards include the `nutsnews-deployment` annotation stream. Deployment workflows should append promotion, rollback, failover, and database-provider events through the Grafana annotations API; Terraform-managed `grafana_annotation` is intentionally not used because it updates one stateful event rather than retaining append-only history. Database-provider start and outcome annotations cover every applied provider mutation, including an apply where the optional Vercel release dispatch is disabled. Final promotion annotations are dispatched as a separate workflow run: an annotation API outage leaves that observability run failed with a retained `delivery_unverified` receipt, but cannot change an already-authoritative production promotion result or cause the production mutation to be retried. Grafana UI/API publishers accept only the exact query-free `https://kindcantaloupe2036.grafana.net` origin (with optional explicit port 443) before attaching a bearer token; Synthetic Monitoring clients independently require the `synthetic-monitoring-api*.grafana.net` service family and every client refuses redirects. The `NutsNews Current Production Ownership` dashboard observes the routed web target, database provider, and web revision from the canonical `https://www.nutsnews.com/readyz` response after validating status, no-store semantics, bounded identity values, and matching identity headers; it joins that observation with the VPS deployment receipt for the infrastructure revision. The ingestion-owner and worker mode/write-gate cards consume the backend host's protected deployment signal, validate its mode/expected-active pair, and gate it on exporter freshness. This signal reflects the protected deployment configuration; a future direct read of `worker_uplift_final.cutover_control` is required before calling it database-confirmed cutover state. The remaining cards show backend API revision, host-verified version/revision/running-image-digest identity for all eight workers, runtime deployment/adapter identity, ownership-aware readiness, and source freshness. Until telemetry-enabled worker images are published and repinned, the runtime deployment and readiness cards explicitly show that rollout dependency rather than inferring health from the host image inventory. Shadow worker readiness renders as an explicit disabled state; only production-owned workers are required to report `outcome="ok"`.
...
Run the protected `Grafana Cloud Plan` workflow first. It performs a normal plan and inspects the refresh-only plan's machine-readable `resource_drift`. Root output-only state changes and Grafana's exact empty notification-timing normalization are ignored; non-empty timings and every other remote resource drift still fail closed and must be reconciled before applying.
