...
The `integration---linux-node` inventory distinguishes 24 vendor alert rules from 16 recording rules. Thirty-five rules are long-term retained and integration-owned. The five legacy `asserts-node.rules` recording rules remain temporarily because the authenticated live integration API reports Linux Server 1.6.2 installed, 1.6.2 available, and `has_update=false`; the expected 1.6.3 upgrade is not offered to this stack. The verifier accepts only the exact reviewed 40-rule unavailable-upgrade shape or the exact 35-rule supported post-upgrade shape and rejects partial or unknown replacements. Never delete the five recording rules by UID. Re-review the catalog and use Grafana's supported integration upgrade once the live API offers it.

The 24 vendor alerts are not exempt from the universal NutsNews label and annotation contract. The protected `Grafana Linux Integration Alert Normalization` workflow updates only their reviewed UIDs through Grafana's Alerting Provisioning API, preserves vendor query definitions/provenance, changes no recording rules, and reads every rule back after write. It applies normalized `severity`, `owner`, `route`, `service`, `deployment_environment`, dashboard, and runbook context. Any UID, identity, provenance, source-severity, query, or integration-version drift fails closed. Re-run this workflow after a supported integration upgrade if the vendor refresh removes the NutsNews context. Separately, the catalog starts with `baselineStatus=pending_authenticated_rollout`: the first authenticated verification exports deterministic definition hashes and fails closed. Review the live definitions, commit `definitionFingerprintSha256` for all 35 long-term retained rules, and set the status to `approved`; only subsequent matching runs validate definition drift.
