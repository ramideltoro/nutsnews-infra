locals {
  ...
  node_exporter_metric_filter = "job=~\"integrations/unix\", instance=~\"$instance\""
  ...
  dashboard_specs = {
    ...
    production_ownership = {
      ...
      panels = [
        { title = "Web target", type = "stat", datasource = "prometheus", unit = "short", width = 6, height = 8, description = "Routed production ownership observed from the validated canonical /readyz response.", noValue = "Unavailable — canonical production ownership observation is missing or stale", expr = "max by (web_target) (nutsnews_production_ownership_info{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"}) and on() (max(nutsnews_production_ownership_available{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"}) == 1) and on() ((time() - max(nutsnews_production_ownership_last_success_timestamp_seconds{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"})) < 300)" },
        { title = "Database provider", type = "stat", datasource = "prometheus", unit = "short", width = 6, height = 8, description = "Routed database ownership observed from the validated canonical /readyz response.", noValue = "Unavailable — canonical production ownership observation is missing or stale", expr = "max by (database_provider) (nutsnews_production_ownership_info{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"}) and on() (max(nutsnews_production_ownership_available{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"}) == 1) and on() ((time() - max(nutsnews_production_ownership_last_success_timestamp_seconds{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"})) < 300)" },
        ...
        { title = "Ownership telemetry available", type = "stat", datasource = "prometheus", unit = "short", width = 6, height = 8, description = "Combined ownership-source validity and freshness: canonical routed readiness and the backend protected deployment signal must both be current and valid.", noValue = "Unavailable — ownership source availability telemetry is missing", expr = "(max(nutsnews_production_ownership_available{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"}) or vector(0)) * (max(nutsnews_backend_worker_uplift_ownership_available{job=\"nutsnews-backend-host\",instance=\"backend.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host\",host=\"backend.nutsnews.com\",environment=\"${var.deployment_environment}\",deployment_environment=\"${var.deployment_environment}\"}) or vector(0)) * (max(nutsnews_backend_metric_exporter_available{job=\"nutsnews-backend-host\",instance=\"backend.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host\",host=\"backend.nutsnews.com\",environment=\"${var.deployment_environment}\",deployment_environment=\"${var.deployment_environment}\"}) or vector(0)) * (((time() - max(nutsnews_production_ownership_last_success_timestamp_seconds{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"})) < bool 300) or vector(0)) * (((time() - max(nutsnews_backend_metric_scrape_timestamp_seconds{job=\"nutsnews-backend-host\",instance=\"backend.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host\",host=\"backend.nutsnews.com\",environment=\"${var.deployment_environment}\",deployment_environment=\"${var.deployment_environment}\"})) < bool 600) or vector(0))" },
        { title = "Ownership telemetry freshness", type = "stat", datasource = "prometheus", unit = "s", width = 6, height = 8, description = "Age of the latest successful canonical readiness ownership observation; failed validation is unavailable, never a stale desired-state claim.", noValue = "Unavailable — canonical ownership observation has no valid success timestamp", expr = "time() - max(nutsnews_production_ownership_last_success_timestamp_seconds{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",service=\"host-exporter\",host=\"vps.nutsnews.com\",deployment_environment=\"${var.deployment_environment}\"} > 0)" },
      ]
    }
  }
  ...
  log_pipeline_alert_rules = {
    ...
    alloy_readiness = {
      ...
      datasource    = "prometheus"
      expr          = "(1 - (max(nutsnews_alloy_ready{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"}) or vector(0))) + (1 - (max(nutsnews_alloy_readiness_probe_success{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"}) or vector(0))) + (1 - (max(nutsnews_alloy_ready{job=\"nutsnews-backend-host\",instance=\"backend.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"}) or vector(0))) + (1 - (max(nutsnews_alloy_readiness_probe_success{job=\"nutsnews-backend-host\",instance=\"backend.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"}) or vector(0)))"
      threshold     = 0
      for_period    = "5m"
      severity      = "critical"
      no_data_state = "Alerting"
    ...
    observability_collector_stale = {
      ...
      datasource    = "prometheus"
      expr          = "(((time() - max(nutsnews_observability_textfile_last_success_timestamp_seconds{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"})) > bool 300) or max(absent(nutsnews_observability_textfile_last_success_timestamp_seconds{job=\"integrations/unix\",instance=\"vps.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"})) or vector(0)) + (((time() - max(nutsnews_backend_metric_scrape_timestamp_seconds{job=\"nutsnews-backend-host\",instance=\"backend.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"})) > bool 600) or max(absent(nutsnews_backend_metric_scrape_timestamp_seconds{job=\"nutsnews-backend-host\",instance=\"backend.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"})) or vector(0)) + (1 - (max(nutsnews_backend_metric_exporter_available{job=\"nutsnews-backend-host\",instance=\"backend.nutsnews.com\",service_namespace=\"nutsnews\",deployment_environment=\"${var.deployment_environment}\"}) or vector(0)))"
      threshold     = 0
      for_period    = "5m"
      severity      = "major"
      no_data_state = "Alerting"
