...
The Ops Portal layer adds a read-only dashboard served by the local Caddy foundation on `127.0.0.1:8080` and protected by Google OAuth for `rami.deltoro@gmail.com` only. A local systemd timer collects sanitized host, Docker, service, log, security, backup, alert, GitOps, and external free-tier usage status into `/opt/nutsnews/portal-assets/data/status.json`. The portal has no direct management buttons; future actions must still go through PR review and protected apply workflows.
